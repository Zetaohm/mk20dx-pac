...
impl W {
    ...
    #[inline(always)]
    #[must_use]
    pub fn avbuschg(&mut self) -> AvbuschgW<OtgistatSpec> {
        AvbuschgW::new(self, 0)
    }
    #[doc = "Bit 2 - This bit is set when a change in VBUS is detected on a B device."]
    #[inline(always)]
    #[must_use]
    pub fn b_sess_chg(&mut self) -> BSessChgW<OtgistatSpec> {
        BSessChgW::new(self, 2)
    }
    #[doc = "Bit 3 - This bit is set when a change in VBUS is detected indicating a session valid or a session no longer valid"]
    #[inline(always)]
    #[must_use]
    pub fn sessvldchg(&mut self) -> SessvldchgW<OtgistatSpec> {
    ...
    #[inline(always)]
    #[must_use]
    pub fn idchg(&mut self) -> IdchgW<OtgistatSpec> {
        IdchgW::new(self, 7)
    }
}
